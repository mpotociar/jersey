    ...
    public ClientRuntime(final ClientConfig config, final Connector connector, final ServiceLocator locator) {
        ...
        this.asyncRequestExecutor = Values.lazy(new Value<ExecutorService>() {
            ...
            public ExecutorService get() {
                return locator.getService(ExecutorService.class, DefaultClientAsyncExecutorProvider.ClientAsyncExecutor.INSTANCE);
            }
        });
        ...
        if (closed.compareAndSet(false, true)) {
            try {
                ...
                try {
                    ...
                    try {
                        if (asyncRequestExecutor.isInitialized()) {
                            asyncRequestExecutor.get().shutdownNow();
                        }
                    } finally {
