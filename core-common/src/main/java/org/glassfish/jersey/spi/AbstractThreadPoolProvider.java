...
public abstract class AbstractThreadPoolProvider<E extends ThreadPoolExecutor> {
    ...
    private static final ExtendedLogger LOGGER =
            new ExtendedLogger(Logger.getLogger(AbstractThreadPoolProvider.class.getName()), Level.FINEST);
    ...
    protected final E getExecutor() {
        if (isClosed()) {
            throw new IllegalStateException("Thread pool executor provider has been closed.");
        }
    ...
    public final void close() {
        ...
        try {
            ...
            if (lazyExecutorServiceProvider.isInitialized()) {
                shutdownExecutor(lazyExecutorServiceProvider.get());
            }
        }
    }
    ...
    private void shutdownExecutor(final ExecutorService executorService) {
        ...
        try {
            terminated = executorService.awaitTermination(getTerminationTimeout(), TimeUnit.MILLISECONDS);
        } catch (InterruptedException e) {
            if (LOGGER.isDebugLoggable()) {
                LOGGER.log(
                        LOGGER.getDebugLevel(),
                        "Interrupted while waiting for thread pool executor " + name + " to shutdown.",
                        e);
            }
            interrupted = true;
        }
        ...
        try {
            if (!terminated) {
                final List<Runnable> cancelledTasks = AccessController.doPrivileged(new PrivilegedAction<List<Runnable>>() {
